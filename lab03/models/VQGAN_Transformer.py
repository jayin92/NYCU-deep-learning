...
# TODO3 step1-1: define one iteration decoding
    ...
    def inpainting(self, z_indices, mask, mask_num, ratio, mask_func):
        ...
        # B x num_image_tokens x num_codebook_vectors
        logits = self.transformer(masked_z_indices)
        # Apply softmax to convert logits into a probability distribution across the last dimension.
        probs = logits.softmax(dim=-1)
        # Get the predicted probabilities for the masked tokens
        z_indices_predict = torch.distributions.Categorical(
            logits=logits).sample()  # sample from the predicted distribution
        while torch.any(z_indices_predict == self.mask_token_id):
            z_indices_predict = torch.distributions.Categorical(
                logits=logits).sample()

        # FIND MAX probability for each token value
        # keep the original tokens for unmasked positions
        z_indices_predict[~mask] = z_indices[~mask]
        # B x num_image_tokens
        z_indices_predict_prob = probs.gather(-1,
                                              z_indices_predict.unsqueeze(-1)).squeeze(-1)
        z_indices_predict_prob = torch.where(mask, z_indices_predict_prob, torch.full_like(
            z_indices_predict_prob, float('inf')))  # set unmasked positions to inf

        # apply the mask function to get the ratio
        ...
        g = torch.distributions.Gumbel(0, 1).sample(z_indices_predict_prob.shape).to(
            z_indices_predict_prob.device)  # gumbel noise
        temperature = self.choice_temperature * (1 - ratio)
        confidence = z_indices_predict_prob + temperature * g
        sorted_confidence, _ = torch.sort(
            confidence, dim=-1)  # sort the confidence scores
        threshold = sorted_confidence[:, mask_len].unsqueeze(-1)
        # new mask has confiedence less than the threshold
